...
int main() {
	...
	/*else {
		for (output_arr[3] =0 ; output_arr[3]<=9; output_arr[3]++){
			for (output_arr[2] =0 ; output_arr[2]<=9; output_arr[2]++){
				for (output_arr[1] =0 ; output_arr[1]<=9; output_arr[1]++){
					for (output_arr[0] =0 ; output_arr[0]<=9; output_arr[0]++){
						for (i = 0; i<4;i++){
							LPC_GPIO0->FIOPIN = i<<15;
							LPC_GPIO0->FIOPIN |= array_dec[output_arr[i]]<<4;
							for (j =0; j<1200; j++);
